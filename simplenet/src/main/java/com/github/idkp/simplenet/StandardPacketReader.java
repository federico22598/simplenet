...
public final class StandardPacketReader implements PacketReader {
    ...
    public StandardPacketReader(SocketChannel channel, ConnectionConfiguration connConfig) {
    ...
    public ReadResult read() throws IOException {
        if (!active) {
            ReadResult headerReadResult = readHeader(connConfig);
        ...
        Collection<PacketListener<?>> listeners = connConfig.getPacketListeners(packetId);

        if (listeners != null) {
            for (PacketListener<?> listener : listeners) {
                //noinspection unchecked
                ((PacketListener<Object>) listener).accept(payload);
            }
        }
    ...
    private ReadResult readHeader(ConnectionConfiguration connConfig) throws IOException {
        active = true;
